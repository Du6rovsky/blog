...
},{"./js/bootstrap.js":5,"./js/localstorage.js":7,"vue":3,"vue-router":2}],5:[function(require,module,exports){
'use strict';
...
},{"../modules/blog/blog-template.js":8,"../modules/post/post-template.js":9,"./config.js":6}],6:[function(require,module,exports){
'use strict';
...
module.exports={template:'<div class="blog"><span v-if="posts.length == 0" class="label label-warning">Blog is empty. Please, create a new post.</span><form v-on:submit="addPost"><input v-model="newPost" type="text" placeholder="Type your post - theme#post" maxlength="140" required="required" class="form-control"></form><hr><ul id="posts" v-if="posts.length &gt; 0" class="list-group"><div v-if="posts.length &gt; 1" class="filter"><i class="pull-left fa fa-filter fa-lg"></i> <input v-model="filter" type="text" placeholder="Filter" maxlength="30" class="form-control"><hr></div><li v-for="post in posts | filterBy filter" class="list-group-item"><div class="comments pull-left"><i class="fa fa-comments"></i>{{ post.comments.length }}</div><span v-if="post.theme" class="theme label label-success">{{post.theme}}</span><a v-link="{name: &quot;post&quot;, params: { postId: post.id }}">{{ post.content }}</a><i v-on:click="removePost(post.id)" class="trash pull-right fa fa-trash"></i></li></ul></div>'}
},{}],9:[function(require,module,exports){
module.exports={template:'<div v-for="p in posts | filterBy $route.params.postId" class="post"><a href="/blog" class="pull-left"><i class="fa fa-arrow-left fa-lg"></i></a><i v-on:click="edit = !edit" class="edit fa fa-pencil-square-o fa-lg"></i><br><h3 v-show="!edit" v-html="p.content">{{p.content}}</h3><input type="text" v-model="p.content" v-on:keyup.enter="edit = false" v-show="edit" minlength="10" class="form-control"><span v-if="p.theme" class="label label-success">{{p.theme}}</span><hr><div class="comments"><span v-if="!p.comments[0]" class="label label-warning">Comments are not found. Do you want to be first?<br></span><form v-on:submit="addComment($event, $route.params.postId)"><input v-model="newComment" type="text" placeholder="Type your comment" maxlength="60" required="required" class="form-control"></form><hr><ul id="comments" class="list-group"><li v-for="comment in p.comments" class="list-group-item"><i class="comment pull-left fa fa-comment"></i>{{ comment }}<i v-on:click="removeComment(p.id, $index)" class="trash pull-right fa fa-trash"></i></li></ul></div></div>'}
},{}]},{},[4]);
